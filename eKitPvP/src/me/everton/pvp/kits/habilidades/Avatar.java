...
public class Avatar implements Listener {
	...
	public void Trocar(PlayerInteractEvent e) {
		...
		if (KitManager.getKit(p) == KitType.AVATAR) {
			...
			} else if ((Terra2.contains(p.getName()))
					&& (Terra2.contains(p.getName()))
					&& ((e.getAction() == Action.LEFT_CLICK_AIR) || (e
							.getAction() == Action.LEFT_CLICK_BLOCK))
	...
	public void AvatarAr(PlayerInteractEvent e) {
		...
		if (KitManager.getKit(p) == KitType.AVATAR) {
			...
			if ((Ferro2.contains(p.getName()))
				...
				if (!cd.containsKey(p.getName())) {
					...
					Vector Ferro = p.getLocation().getDirection().normalize()
							.multiply(55);
					Snowball FerroH = (Snowball) p
							.launchProjectile(Snowball.class);
					FerroH.setVelocity(Ferro);
					FerroH.setMetadata("Ar",
					...
					while (Ferrao.hasNext()) {
						...
						p.playSound(p.getLocation(), Sound.STEP_WOOL, 5.5F,
								5.5F);
						p.playSound(p.getLocation(), Sound.STEP_WOOL, 1.5F,
								1.5F);
						p.playSound(p.getLocation(), Sound.STEP_WOOL, 2.5F,
								2.5F);
						p.playSound(p.getLocation(), Sound.STEP_WOOL, 3.5F,
								3.5F);
						p.playSound(p.getLocation(), Sound.STEP_WOOL, 4.5F,
								4.5F);
		...
		if ((KitManager.getKit(p) == KitType.AVATAR)
			...
			if (!cd.containsKey(p.getName())) {
				...
				Vector Agua = p.getLocation().getDirection().normalize()
						.multiply(55);
				Snowball AguaH = (Snowball) p.launchProjectile(Snowball.class);
				AguaH.setVelocity(Agua);
				AguaH.setMetadata("Agua", new FixedMetadataValue(Main.plugin,
		...
		if ((Terra2.contains(p.getName()))
				&& ((e.getAction() == Action.RIGHT_CLICK_AIR) || (e.getAction() == Action.RIGHT_CLICK_BLOCK))
				&& (p.getItemInHand().getType() == Material.GRASS)) {
			e.setCancelled(true);
			if (!cd.containsKey(p.getName())) {
				API.startCd(p, 30, 0, cd, "§7[§c!§7] O cooldown acabou!");
				Vector Terra = p.getLocation().getDirection().normalize()
						.multiply(55);
				Snowball TerraH = (Snowball) p.launchProjectile(Snowball.class);
				TerraH.setVelocity(Terra);
				TerraH.setMetadata("Terra", new FixedMetadataValue(Main.plugin,
						Boolean.valueOf(true)));
				...
				p.sendMessage("§7[§c!§7] Kit em cooldown de §c§l" + cd.get(p.getName()) + " segundos!");
			}
		}
		...
		if ((Red2.contains(p.getName()))
			...
			if (!cd.containsKey(p.getName())) {
				...
				Vector Fogo = p.getLocation().getDirection().normalize()
						.multiply(55);
				Snowball FogoH = (Snowball) p.launchProjectile(Snowball.class);
				FogoH.setVelocity(Fogo);
				FogoH.setMetadata("Fogo", new FixedMetadataValue(Main.plugin,
						Boolean.valueOf(true)));
	...
	public void EntityDamageEvent(EntityDamageByEntityEvent e) {
		...
		if ((e.getDamager() instanceof Snowball)) {
			...
			if (Tomou.hasMetadata("Ar")) {
				e.setDamage(16.0D);
				((LivingEntity) Tomou2).addPotionEffect(new PotionEffect(
